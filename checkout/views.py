...
def cache_checkout_data(request):
    """
    View that caches checkout data
    """
...
def checkout(request):
    """
    View that handles the user checkout process
    """
    ...
    if request.method == 'POST':
        ...
        bag = request.session.get('bag', {})
        if not bag:
            messages.error(request, "Wait a minute - you've no espresso in your order! \
                                Head back and add an espresso or two!")
            return redirect(reverse('products'))

        current_bag = basket_contents(request)
        total = current_bag['basket_total']
...
def checkout_success(request, order_number):
    """
    View that handles checkout success
    """
